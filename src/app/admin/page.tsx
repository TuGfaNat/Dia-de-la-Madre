...
export default function AdminPage() {
  ...
  // Estados del CRUD de categorías
  ...
  const fetchCategories = async () => {
    try {
      ...
      setErrorCategories(e.message || 'No se pudieron cargar las categorías');
    } finally {
  ...
  // Función para subir múltiples imágenes a la base de datos (local o Vercel Blob)
    ...
    try {
      ...
      if (formType === 'add') {
        ...
        setEditingProduct(prev => {
          if (!prev) return null;
          const currentImages = prev.images || [prev.image];
  ...
  const handleRemoveAddImage = (indexToRemove: number) => {
    setNewProduct(prev => {
  ...
  const handleUpdateCategory = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchCategories();
        fetchProducts(); // Actualizar productos para que reflejen el nuevo nombre de categoría
      } else {
        const err = await res.json();
  ...
  const handleDeleteCategory = async (id: string, name: string) => {
    if (!confirm(`¿Estás seguro de eliminar la categoría "${name}"?\nTodos los productos de esta categoría se reasignarán automáticamente a la categoría "General".`)) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        fetchCategories();
        fetchProducts(); // Actualizar productos reasignados
      } else {
        const err = await res.json();
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    const { name, description, price, stock, image, images, category } = newProduct;

    // A product needs at least one image, either from 'images' array or legacy 'image'
    const hasImages = (images && images.length > 0) || (image && image.trim() !== '');

    if (!name || !description || !price || !stock || !category || !hasImages) {
      setAddError('Por favor completa todos los campos (nombre, descripción, precio, stock, categoría y al menos una imagen)');
      setAddLoading(false);
      return;
    }

    const finalImages = images && images.length > 0 ? images : [image];
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewProduct({
          ...
          image: '',
          images: [],
          category: categoriesList.length > 0 ? categoriesList[0].name : 'Flores'
        });
      } else {
      ...
      <main className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 flex-grow">
        ...
        <div className="flex gap-2 border-b border-card-border pb-2">
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-xs font-bold tracking-wide transition-all cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'products'
                ? 'bg-rose-500 text-white shadow-md'
                : 'bg-card-bg text-txt-secondary border border-card-border hover:border-rose-450 hover:text-rose-500'
            }`}
          >
          ...
          >
            <Grid className="w-4 h-4" /> Categorías
          </button>
        </div>
        ...
        {activeTab === 'categories' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Formulario Agregar Categoría (Columna 1) */}
            <div className="md:col-span-1 bg-card-bg p-6 rounded-2xl border border-card-border shadow-xs space-y-4 h-fit">
              ...
              <form onSubmit={handleCreateCategory} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-txt-muted uppercase mb-2">Nombre de la Categoría</label>
                  <input
                    type="text"
                    required
                    value={newCategoryName}
                    onChange={(e) => setNewCategoryName(e.target.value)}
                    placeholder="Ej: Globos, Regalos Personalizados"
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm shadow-rose-100/10"
                >
            ...
            {/* Listado de Categorías (Columna 2-3) */}
            <div className="md:col-span-2 bg-card-bg rounded-2xl border border-card-border shadow-xs overflow-hidden">
              <div className="p-6 border-b border-card-border">
                <h2 className="text-base font-bold text-txt-primary">Categorías Registradas</h2>
                <p className="text-xs text-txt-muted mt-1 font-medium">
                  Gestiona las categorías de la tienda. Al editar o eliminar se actualizarán los productos relacionados automáticamente.
                </p>
              </div>
              ...
              {loadingCategories ? (
                <div className="p-20 text-center">
                  <Loader2 className="w-8 h-8 text-rose-500 animate-spin mx-auto mb-2" />
                  <p className="text-txt-muted text-sm font-bold">Cargando categorías...</p>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-card-border/5 text-[10px] font-bold uppercase tracking-wider text-txt-muted border-b border-card-border">
                        ...
                        <th className="py-4 px-6 w-32">Identificador</th>
                        <th className="py-4 px-6 w-40 text-center">Acciones</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-card-border/30">
                      {categoriesList.map((cat) => {
                        ...
                        return (
                          <tr key={cat.id} className="hover:bg-card-border/5 transition-colors text-xs font-medium text-txt-secondary">
                            ...
                            <td className="py-4 px-6">
                              {isEditing ? (
                                <input
                                  ...
                                  className="w-full px-3 py-1.5 text-xs border border-card-border rounded-lg bg-card-bg focus:outline-none focus:ring-2 focus:ring-rose-450 text-txt-primary font-bold"
                                  autoFocus
                                />
                              ) : (
                                <span className="font-bold text-txt-primary text-sm">{cat.name}</span>
                              )}
                            </td>

                            {/* Identificador (slug) */}
                            <td className="py-4 px-6">
                              <span className="text-[10px] px-2 py-0.5 bg-card-border/10 text-txt-muted rounded font-mono font-bold uppercase">
                                {cat.id}
                              </span>
                            </td>
          ...
          <div className="bg-card-bg rounded-3xl w-full max-w-xl border border-card-border shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleAddProduct} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-txt-muted uppercase mb-1">Imágenes del Producto (Subir múltiples)</label>
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-grow px-3 py-1.5 border border-card-border bg-card-bg rounded-lg focus:outline-none focus:ring-1 focus:ring-rose-450 text-xs text-txt-primary font-medium"
                      />
                      <button
                        type="button"
                        onClick={() => {
                          ...
                          if (val) {
                            setNewProduct(prev => {
                              const updatedImages = [...(prev.images || []), val];
                              return {
                                ...prev,
                                images: updatedImages,
                                image: updatedImages[0] || prev.image
                        ...
                        className="px-3 py-1.5 bg-rose-500/10 text-rose-500 hover:bg-rose-500/20 rounded-lg text-xs font-bold transition-all cursor-pointer border border-rose-500/20 shrink-0"
                      >
                        Añadir
                      </button>
                    </div>
                    ...
                    {newProduct.images && newProduct.images.length > 0 && (
                      <div className="flex flex-wrap gap-2.5 pt-2">
                        {newProduct.images.map((imgUrl, index) => (
                          <div key={index} className="relative w-20 h-20 rounded-xl overflow-hidden border border-card-border group shadow-xs">
                            <img src={imgUrl} alt={`Vista previa ${index + 1}`} className="w-full h-full object-cover" />
                            {index === 0 && (
                              <span className="absolute top-1 left-1 bg-rose-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded shadow">
                                Principal
                              </span>
                            )}
                            <button
                              ...
                              className="absolute top-1 right-1 p-1 bg-black/75 hover:bg-rose-600 text-white rounded-full transition-colors cursor-pointer"
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const input = document.getElementById('editUrlInput') as HTMLInputElement;
                          const val = input?.value.trim();
                          if (val) {
                            setEditingProduct(prev => {
                              if (!prev) return null;
                              const currentImages = prev.images || [prev.image];
                              const updatedImages = [...currentImages, val];
                              return {
                                ...prev,
                                images: updatedImages,
                                image: updatedImages[0] || prev.image
